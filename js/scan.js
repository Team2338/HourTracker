...
function onFoundBarcode(result){
	...
	if (result.length == 8){
		...
		document.querySelector('#greenBox').style.display = "block";
		...
		document.querySelector('#greenBox').style.display = "none";
...
function logClockIn(ID){
	...
	console.log('logging Check In'+ ID);
	timeIn = new Date();
	console.log(timeIn);
	...
	monthString = (monthString.length === 1)? '0' + dayString : dayString;
	dayString = (dayString === 1)? '0' + dayString : dayString;
...
function logClockOut(ID){
	...
	console.log('logging Check OUT'+ ID);
	timeOut = new Date();
	console.log(timeOut);
